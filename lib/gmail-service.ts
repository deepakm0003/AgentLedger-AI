...
export async function sendEmailWithGmail(alert: EmailAlert): Promise<boolean> {
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      secure: false,
      auth: {
        user: process.env.GMAIL_USER || 'deepak23188@iiitd.ac.in',
        pass: process.env.GMAIL_PASS || 'vrpq fpuy btxd tckz',
      },
    })
...
// Alternative: Use a free email service like EmailJS or Resend
export async function sendEmailWithResend(alert: EmailAlert): Promise<boolean> {
  try {
    ...
    if (response.ok) {
      ...
      console.log('Email sent with Resend:', result.id)
      return true
    } else {
...
// Simple email service using a webhook
export async function sendEmailWithWebhook(alert: EmailAlert): Promise<boolean> {
  try {
    const webhookUrl = process.env.EMAIL_WEBHOOK_URL || 'https://hooks.slack.com/services/YOUR/SLACK/WEBHOOK'
    
    const payload = {
      text: `🚨 ${alert.riskLevel} Risk Transaction Alert`,
      blocks: [
        {
          type: 'header',
          text: {
        ...
        },
        {
          ...
          fields: [
            {
              type: 'mrkdwn',
              text: `*Amount:* $${alert.transaction.amount.toLocaleString()}`
            },
            {
              type: 'mrkdwn',
            ...
            {
              type: 'mrkdwn',
              text: `*Transaction ID:* ${alert.transaction.id}`
            },
            {
              type: 'mrkdwn',
              text: `*User Email:* ${alert.to}`
            }
          ]
        },
        {
          ...
          text: {
            type: 'mrkdwn',
            text: `*Description:* ${alert.transaction.description}`
          }
        },
        {
          type: 'section',
          text: {
            type: 'mrkdwn',
            text: `*AI Analysis:* ${alert.explanation}`
          }
        }
      ]
    }
